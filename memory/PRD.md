...
## Recently completed
- 2026-02 — **Native iOS app scaffolding (Capacitor 7)**: `capacitor.config.ts` with bundle ID `com.sealtechbuilding.crm`, app icon (1024) + splash (2732) from SealTech logo, camera/GPS/photo-library permission strings, `nativeCapabilities.ts` helper (dynamic Capacitor plugin imports for camera + GPS), Codemagic `codemagic.yaml` for cloud iOS builds (no Mac needed), setup playbook at `/app/NATIVE_APP_SETUP.md`. **Blocked on user's Apple Developer account signup ($99/yr).**
