...
def compute_aging(invoices: list, as_of: date) -> dict:
    """Bucket each open invoice's balance_due into aging windows.
    Returns a dict with buckets + grand total. `as_of` is the statement date.
    """
...
def build_statement_pdf(customer: dict, invoices: list, statement_date_iso: str) -> bytes:
    ...
    if len(table_data) == 1:
        ...
        detail_table = Table(
            table_data,
            colWidths=[0.95 * inch, 0.85 * inch, 0.85 * inch, 1.85 * inch, 0.85 * inch, 0.75 * inch, 0.85 * inch, 0.55 * inch],
        )
        ...
        # Force header row text color (after rowbackgrounds)
        detail_table.setStyle(TableStyle([
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ]))
        story.append(Paragraph("INVOICE DETAIL", s["label"]))
    ...
    if grand_balance > 0.01:
        msg = (
            f'<b>Please remit payment of <font color="#1D4ED8">{_currency(grand_balance)}</font> at your earliest convenience.</b>  '
